...
class MvtxNoiseMap
{
 public:
  ...
  explicit MvtxNoiseMap(std::vector<std::map<int, int>>& noise) { mNoisyPixels.swap(noise); }
  ...
  static int getKey(uint32_t row, uint32_t col) { return (row << SHIFT) + col; }
  static int key2Row(uint32_t key) { return key >> SHIFT; }
  static int key2Col(uint32_t key) { return key & MASK; }
  static uint32_t getChipId(uint8_t layId, uint8_t stvId, uint8_t chipId)
  {
    return nChipsPerLayerZero[layId] + stvId * 9 + chipId;
  }

 private:
  static constexpr uint8_t nChipsPerLayerZero[3] = {0, 12 * 9, 16 * 9};
